...
def create_bins(df, nobins=10, bintype="equal-width"):
    ...
    for col in df1:
        if df1[col].dtype == "int64" or df1[col].dtype == "float64" and col not in ['CLASS', 'ID']:
            ...
            df1[col] = df1[col].cat.set_categories([x for x in range(nobins)])
...
def apply_bins(df, binning):
    ...
    for col in df1:
        if col in binning.keys():
            df1[col] = pd.cut(df1[col], binning[col])
            df1[col].astype('category')
            df1[col] = df1[col].cat.set_categories([x for x in range(len(binning.keys()))])
